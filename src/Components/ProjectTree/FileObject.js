...
const FileObject = ({ fileName }) => {
  ...
  const fileHandler = (e) => {
    ...
    const url = "http://13.233.94.116:8000/setfile/" + fileName;
    axios.post(url).then((response) => {
    });
  };

  const deleteFile = () => {
    const url = "http://13.233.94.116:8000/api/delete/" + myContext.currentFile;
  ...
  return fileName === myContext.currentFile ? (
    <div className="container inline-flex flex-row relative bg-Vintage rounded-tl-2xl rounded-bl-2xl ml-2">
      <div
